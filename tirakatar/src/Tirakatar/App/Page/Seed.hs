...
module Tirakatar.App.Page.Seed(
    mnemonicPage
  , mnemonicWidget
  , seedRestorePage
  , seedRestoreWidget
...
checkPage mnemonic = wrapperSimple True $ do
  ...
  void $ nextWidget $ ffor mnemonicE $ \mnemonic' -> Retractable {
      retractableNext = setupDispatcher AccountRestored mnemonic'
    , retractablePrev = Just $ pure $ checkPage mnemonic'
    }

setupDispatcher :: MonadFrontBase t m => AccountSource -> Mnemonic -> m ()
setupDispatcher accs mnem = if isAndroid
  then setupLoginPage AccountRestored mnem
  else setupPasswordPage AccountRestored mnem Nothing
...
askSeedPasswordPage encryptedMnemonic = do
  ...
  void $ nextWidget $ ffor (decodeUtf8With lenientDecode <$> verifiedMnemonicE) $ \mnem -> Retractable {
      retractableNext = setupDispatcher AccountRestored mnem
...
base58RestorePage = wrapperSimple True $ mdo
  ...
  let validationE = poke submitE $ \_ -> do
        ...
        pure $ maybe (Left [SPSMnemonicDecodeError]) Right $
          (eitherToMaybe . S.decode <=< decodeBase58Check) encodedEncryptedMnemonic
