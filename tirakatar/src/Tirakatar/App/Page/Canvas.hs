module Tirakatar.App.Page.Canvas(
    elementPosition
  , rawJSCall
  -- JS Raw Calls
  , drawGridT
...
elementPosition e = liftJSM $ do
  void $ eval ("tirakatar_elementPosition = function(a) { return a.getBoundingClientRect(); }" :: Text)
  jsv <- liftJSM $ jsg1 ("tirakatar_elementPosition" :: Text) (toJSVal e)
...
rawJSCall e t = liftJSM $ do
  ...
  where
    (funcName :: Text) = "tirakatar_drawgrid"
    (func :: Text) = " tirakatar_drawgrid = function(cnv) { " <> " var ctx = cnv.getContext(\"2d\");" <> t <> " }"
...
rawGetCanvasJpeg canvEl CanvasOptions{..} = liftJSM $ do
  ...
  where
    (funcName :: Text) = "tirakatar_canvas_image_data"
    (func :: Text) = " tirakatar_canvas_image_data = function(cnv) {"
