...
void android_copy_str(jobject activity, const char* str) {
  ...
  jstring clipLabel = (*env)->NewStringUTF(env, "tirakatar_clipboard");
  assert(clipLabel);
...
jstring android_paste_str(jobject activity) {
  ...
  jclass clipboardClass = (*env)->FindClass(env, "org/tirakatar/Clipboard");
...
void android_log_write(const char* str) {
  __android_log_write(ANDROID_LOG_DEBUG, "tirakatar", str);
}
