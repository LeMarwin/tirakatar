...
module Tirakatar.App.Android.Native.MainWidget
  ( startMainWidget
  ) where
...
startMainWidget a cbChan url jsm = do
  ...
  BS.useAsCString url $ \curl -> do
    ...
    void . forkIO $ fix $ \next -> do
      io <- readChan cbChan
      runHaskell executor =<< new =<< haskellCallbackPtr io
      next
...
foreign import ccall safe "Reflex_Dom_Android_MainWidget_runHaskell" runHaskell :: JSExecutor -> Ptr HaskellCallbackPtr -> IO ()
...
data HaskellCallbackPtr = HaskellCallbackPtr
  { _haskellCallbackPtr_haskellCallback :: FunPtr (IO ())
  }
...
haskellCallbackPtr :: IO () -> IO HaskellCallbackPtr
haskellCallbackPtr io = HaskellCallbackPtr
  <$> wrapIO io

instance Storable HaskellCallbackPtr where
  sizeOf _ = #{size HaskellCallback}
  alignment _ = #{alignment HaskellCallback}
  poke p (HaskellCallbackPtr cb) = #{poke HaskellCallback, haskellCallback} p cb
  peek p = HaskellCallbackPtr
    <$> #{peek HaskellCallback, haskellCallback} p
